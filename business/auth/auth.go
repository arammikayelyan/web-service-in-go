// Package auth provides authenication and authorization support.
package auth
...
// HasRole returns true if the claims has at least one of this roles.
...
// PublicKeyLookup defines the signature of a function to lookup keys.
//
...
func New(algorithm string, lookup PublicKeyLookup, keys Keys) (*Auth, error) {
	...
	// parser := jwt.NewParser( ValidMethods: []string{algorithm},)
...
// AddKey adds a private key and combinatin kid id to our local store.
func (a *Auth) AddKey(privateKey *rsa.PrivateKey, kid string) {
	a.keys[kid] = privateKey
}

// RemoveKeys remove a private key and combinatin kid id to our local store.
func (a *Auth) RemoveKeys(kid string) {
	delete(a.keys, kid)
}
